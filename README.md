...
- 装备字典自动核验：从 DataJ `/equip` 公开结构化数据解析 `equipId → 中文装备名`
- 自动 enrichment 队列：实时发现的新阵容自动进入待补全队列，并记录首次发现、最近出现、优先级和缺失字段
- 部分自动补全：新阵容可自动核验核心英雄、完整阵容、Carry、羁绊、装备 ID、装备名称与装备携带者；只有装备 ID 100% 映射成功才标记装备字段为 verified
- 推荐安全隔离：`needsEnrichment=true` 或缺少可信运营节奏的阵容不会进入局内自动推荐
- 样本量校验：优先使用 DataJ 源站 `sampleCount`；只有结构化数据不可用时才按出场率百分比估算，并按不同样本口径分别校验
- 依赖安全门禁：CI 会阻止 high-severity 的生产依赖漏洞进入主分支
...
3. **DataJ `/comp`**：当前阵容统计与阵容结构适配器，提供表现数据、原始样本数、阵容英雄、Carry、羁绊及装备 ID。
4. **DataJ `/equip`**：公开装备统计与装备名称字典，用于把阵容装备 ID 核验为中文装备名并绑定到携带英雄。
5. **`data/patch-authority.json`**：独立核验后的版本安全锁。
6. **`data/rank-capability.json`**：记录已核验的国服公共段位能力，避免把通用前端枚举误报成真实国服分段覆盖。
...
DataJ `/comp` 公开页面的 Next.js 序列化数据是 V0.2.1 的主要阵容结构入口。适配器优先读取：
...
因此阵容样本量现在优先采用源站 `sampleCount`，装备则通过公开字典完成 ID→名称核验。只有结构化数据临时失效时，阵容文本解析器才会回退到百分比样本估算。

新出现的实时阵容如果本地知识库还没有完整运营信息，会自动进入：
...
- `full / READY`：本地可信知识已补齐，包括运营节奏，可进入实战推荐候选。
- `partial / BUILD ✓`：公开结构化数据已核验阵容英雄、Carry、羁绊、装备 ID、装备名称和装备携带者，但运营节奏仍缺失。
- `partial / ROSTER ✓`：阵容英雄已核验，但装备名称尚未实现完整映射。
- `pending / NEW`：连稳定阵容结构都尚未取得。

装备字段采用严格完整性规则：某套阵容只有在所有公开装备 ID 都成功映射到名称时，`keyItems` 与 `itemCarriers` 才会加入 `enrichmentVerifiedFields`。部分映射不会被冒充为完整出装。

`partial` 与 `pending` 均继续保持 `needsEnrichment=true`，可以出现在 Meta / 冷门雷达中，但不会进入 `/api/recommend` 的局内自动推荐。项目不会根据阵容费用或常见打法凭空生成 `stagePlan`；没有可信来源时宁可保持隔离。
...
V0.2.1 的实时数据、安全门禁、结构化样本、自动阵容结构 enrichment、装备名称/携带者 enrichment 已完成。仍明确保留两个外部数据边界：

1. 铂金+ → 大师+国服独立分段：当前没有找到无需绕过访问控制的稳定公开入口，因此继续保留“全段位数据 + 宗师+能力验证层”。
2. 新阵容 `stagePlan`：当前公开结构化数据未提供可稳定验证的运营节奏，因此新阵容保持 `partial`，不会为了提高覆盖率而生成未经来源核验的实战运营方案。

V0.3 将聚焦商店来牌、装备、强化符文和经济状态输入，以及实时留牌、转阵、D牌和升人口建议；届时会把运营节奏建模为独立、可验证的决策层，而不是把猜测塞进 V0.2.1 数据层。
