# TFTGOLDENCHANCHAN Overlay V0.8

Windows 桌面悬浮助手，基于 Tauri 2 + Vite。

## V0.8 Board Vision Foundation

V0.8.0 在现有本地 OCR 帧之上加入共享的棋盘视觉状态层，不建立第二套推荐器。

- `lib/board-vision.ts` 将横屏画面划分为 BOARD / BENCH / SHOP GUARD 区域
- 从 PP-OCRv5 blocks 中提取高置信英雄文本证据，并按位置区分场上与备战席
- 识别 `2星 / 二星 / ★★`、`3星 / 三星 / ★★★` 等星级文本，并转换为 3 / 9 张等价持有量送入原有 `GameState`
- 使用当前可信 Meta 快照中的装备名称做模糊匹配；装备文本只有在距离和置信度同时达标时才关联到英雄，否则保持 loose candidate
- 最近最多 8 帧做时间衰减 + 组成一致性融合
- 自动覆盖场上必须同时满足：场上识别身份数=当前人口、同一组成连续至少 3 帧、融合置信度至少 80%
- 备战席与装备在 V0.8.0 默认 candidate-first，只有用户点击“应用/合并候选”才写入
- 窄屏 / 竖屏布局标记 unsupported，不自动写入
- 预览画面显示 BOARD / BENCH 诊断区域，方便核对模拟器缩放与区域是否匹配
- 新增 `tft-board-vision-state` 事件，后续本地图像分类器可直接接入同一状态/融合协议

### V0.8.0 明确边界

当前版本已经完成**区域状态协议、多帧稳定、安全写入和文本证据识别**，但不宣称在没有任何英雄名称/装备文字可见时，已经能仅凭 3D 棋子像素稳定识别全部英雄。真正的像素级英雄分类器会作为后续 V0.8.x 接入同一 `board-vision` 协议；在模型达到验证门槛前不会自动污染 `GameState`。
